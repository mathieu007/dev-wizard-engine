...
export async function resolveConfigPaths(
	...
	const errors: string[] = [];
	const entries: ConfigResolutionEntry[] = [];
	...
	if (explicit.length > 0) {
		for (const provided of explicit) {
			...
			if (await pathExists(absolute)) {
				...
				diagnostics.push(
					formatDiagnostic("explicit", cwd, absolute, "found"),
				);
			} else {
				errors.push(
					`Explicit config path not found: ${formatRelative(cwd, absolute)}`,
				);
				diagnostics.push(
					formatDiagnostic("explicit", cwd, absolute, "missing"),
				);
			}
		}
...
async function evaluateRootConfigs({
	cwd,
	diagnostics,
	entries,
	seen,
	...
	for (const candidate of ROOT_CONFIG_CANDIDATES) {
		...
		diagnostics.push(
			formatDiagnostic("root", cwd, absolute, exists ? "found" : "missing"),
		);
		if (exists) {
...
async function evaluateConfigDirectory({
	cwd,
	diagnostics,
	entries,
	seen,
	...
	if (!exists) {
		diagnostics.push(
			"[directory] dev-wizard-config/ (missing)",
		);
		return;
	}
	...
	if (baseIndexes.length > 0) {
		...
		diagnostics.push(
			`[directory] dev-wizard-config/ (base overlay, ${baseIndexes.length} index file${baseIndexes.length === 1 ? "" : "s"})`,
		);
	} else {
	...
	if (environment) {
		...
		if (await pathExists(envDir)) {
			...
			if (envIndexes.length > 0) {
				...
				diagnostics.push(
					`[directory] dev-wizard-config/environments/${environment}/ (overlay, ${envIndexes.length} index file${envIndexes.length === 1 ? "" : "s"})`,
				);
			} else {
				diagnostics.push(
					`[directory] dev-wizard-config/environments/${environment}/ (overlay present, no index files)`,
				);
			}
		} else {
			diagnostics.push(
				`[directory] dev-wizard-config/environments/${environment}/ (missing)`,
			);
		}
	}
	...
	if (includeLocal) {
		...
		if (await pathExists(localDir)) {
			...
			if (localIndexes.length > 0) {
				...
				diagnostics.push(
					`[directory] dev-wizard-config/local/ (overlay, ${localIndexes.length} index file${localIndexes.length === 1 ? "" : "s"})`,
				);
			}
		}
		...
		if (localRootCandidates.length > 0) {
			...
			diagnostics.push(
				`[root] dev-wizard.config.local.* (overlay, ${localRootCandidates.length} file${localRootCandidates.length === 1 ? "" : "s"})`,
			);
		}
...
async function evaluatePackageJson({
	cwd,
	diagnostics,
	entries,
	seen,
	...
	const wizardConfig = (parsed as { wizard?: { config?: unknown } })?.wizard
		?.config;
	...
	const stringConfigs = configs.filter(
		(item): item is string => typeof item === "string",
	);

	if (stringConfigs.length === 0) {
		errors.push(
			"package.json#wizard.config must be a string or array of strings.",
		);
		return;
	}
	...
	for (const rawPath of stringConfigs) {
		...
		if (await pathExists(absolute)) {
			...
			errors.push(
				`package.json#wizard.config references missing file: ${formatRelative(cwd, absolute)}`,
			);
		}
	}
	...
	if (found === 0) {
		diagnostics.push(
			"[package-json] wizard.config (no existing files found)",
		);
	} else {
		diagnostics.push(
			`[package-json] wizard.config (found ${found} file${found === 1 ? "" : "s"})`,
		);
	}
}
...
function formatRelative(cwd: string, target: string): string {
	const relative = path.relative(cwd, target);
	return relative && !relative.startsWith("..") ? relative : target;
}

async function findDirectIndexFiles(directory: string): Promise<string[]> {
...
async function findRootLocalConfig(cwd: string): Promise<string[]> {
	const results: string[] = [];
	const localCandidates = ROOT_CONFIG_CANDIDATES.map((candidate) =>
		candidate.replace("dev-wizard.config", "dev-wizard.config.local"),
	);
