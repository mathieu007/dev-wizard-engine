...
import type { DevWizardConfig, DevWizardPluginReference } from "./types.js";
...
type ConfigCache = Map<string, DevWizardConfig>;
...
>();
const LEGACY_LIBRARY_WRAPPER_PATTERN =
	/packages[\/\\-]dev-wizard-core[\/\\]examples[\/\\]library[\/\\].+\.wizard\.ya?ml$/u;

export async function loadConfig(
	options: LoadConfigOptions,
): Promise<DevWizardConfig> {
	...
	let aggregated: DevWizardConfig | undefined;
...
interface LoadRecursiveOptions {
	absolutePath: string;
	cwd: string;
	cache: ConfigCache;
...
async function loadConfigRecursive({
	...
	loadingStack,
	onWarning,
}: LoadRecursiveOptions): Promise<LoadedConfig> {
	...
	if (loadingStack.has(absolutePath)) {
		throw new Error(
			`Circular config import detected: ${Array.from(loadingStack).join(
				" -> ",
			)} -> ${absolutePath}`,
		);
	}
	...
	try {
		...
		throw new Error(
			`Failed to read Dev Wizard config at ${absolutePath}: ${String(error)}`,
		);
	}
	...
	if (parsed.plugins?.length) {
		try {
			...
			throw new Error(
				`Failed to resolve plugin module declared in ${absolutePath}: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	}
...
function mergeConfigs(
	target: DevWizardConfig,
	addition: DevWizardConfig,
	additionPath: string,
	onWarning?: (message: string) => void,
): DevWizardConfig {
	...
	for (const scenario of addition.scenarios) {
		if (result.scenarios.some((existing) => existing.id === scenario.id)) {
			throw new Error(
				`Duplicate scenario id "${scenario.id}" while merging ${additionPath}`,
			);
		}
	...
	for (const [flowId, flow] of Object.entries(addition.flows)) {
		if (result.flows[flowId]) {
			throw new Error(
				`Duplicate flow id "${flowId}" encountered while merging ${additionPath}`,
			);
		}
	...
	if (addition.commandPresets) {
		...
		const additionSources =
			COMMAND_PRESET_SOURCES.get(addition) ?? new Map<string, Set<string>>();
		...
		for (const [presetName, presetValue] of Object.entries(addition.commandPresets)) {
			...
			if (existing) {
				if (!isDeepStrictEqual(existing, presetValue)) {
					throw new Error(
						`Duplicate command preset "${presetName}" with conflicting definitions while merging ${additionPath}`,
					);
				}
				onWarning?.(
					`Command preset "${presetName}" defined in ${additionPath} duplicates existing definition from ${existingSources ? Array.from(existingSources).join(", ") : "a previous config"}; using the first definition.`,
				);
			}
			...
			const sources =
				additionSources.get(presetName) ?? new Set<string>([additionPath]);
			const targetSources =
				resultSources.get(presetName) ?? new Set<string>();
	...
	if (addition.plugins?.length) {
		result.plugins ??= [];
		const registeredPlugins = new Map<string, { source?: string }>();
		...
		for (const plugin of addition.plugins) {
			...
			if (registeredPlugins.has(key)) {
				...
				onWarning?.(
					`Plugin "${plugin.module}" from ${additionPath} duplicates an earlier registration${previousSource ? ` defined in ${previousSource}` : ""}; using the first definition.`,
				);
				continue;
			}
...
function cloneConfig(config: DevWizardConfig): DevWizardConfig {
...
function validateSchemaVersion(
	...
	if (schemaVersion !== SUPPORTED_CONFIG_SCHEMA_VERSION) {
		onWarning?.(
			`Config ${filePath} declares schemaVersion ${schemaVersion}, but the current runtime supports ${SUPPORTED_CONFIG_SCHEMA_VERSION}. Behaviour may be undefined.`,
		);
	}
}
...
async function resolveConfigImport({
	specifier,
	fromPath,
	cwd,
	...
	try {
		const packageJsonPath = CONFIG_REQUIRE.resolve(
			path.join(specifier, "package.json"),
			{ paths: [fromDir, cwd] },
		);
		...
		throw new Error(
			`Unable to resolve config import "${specifier}" from ${fromPath}: ${error instanceof Error ? error.message : String(error)}`,
		);
	}

	throw new Error(
		`Unable to resolve config import "${specifier}" from ${fromPath}: no supported config files found`,
	);
}
...
async function locatePackageConfig(pkgRoot: string): Promise<string | undefined> {
...
interface ResolvePluginModuleOptions {
	specifier: string;
	fromPath: string;
	cwd: string;
}
...
function resolvePluginModule({
	...
	fromPath,
	cwd,
}: ResolvePluginModuleOptions): string {
	const fromDir = path.dirname(fromPath);
	...
	try {
		...
		throw new Error(
			`Unable to resolve plugin module "${specifier}" from ${fromPath}: ${error instanceof Error ? error.message : String(error)}`,
		);
	}
}
...
async function fileHasSupportedExtension(filePath: string): Promise<boolean> {
	const extension = path.extname(filePath).toLowerCase();
	return [".yaml", ".yml", ".json", ".json5"].includes(extension);
}

function initializeCommandPresetSources(
	config: DevWizardConfig,
	sourcePath: string,
): void {
...
function ensureCommandPresetSourceMap(
	config: DevWizardConfig,
): Map<string, Set<string>> {
...
function copyCommandPresetSources(
	from: DevWizardConfig,
	to: DevWizardConfig,
): void {
...
function getPluginRegistryKey(plugin: DevWizardPluginReference): string {
	return plugin.resolvedPath ?? plugin.module;
}

export function getCommandPresetSources(
	config: DevWizardConfig,
): Map<string, Set<string>> {
