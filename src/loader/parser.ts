...
const promptDynamicGlobSchema = z.object({
	...
	patterns: z
		...
		.refine(
			...
			"dynamic.patterns must include at least one pattern",
		),
	cwd: z.string().optional(),
...
const autoHandlingSchema = z
	...
	.refine(
		(value) =>
			value.strategy !== "transition" || typeof value.target === "string",
		{
	...
	.refine(
		(value) =>
			value.strategy === "transition" || value.target === undefined,
		{
...
const builtinStepTypeSet = new Set<string>(BUILTIN_STEP_TYPES);
			...
			segments: z
				.array(
					z
						...
						.refine(
							(segment) =>
								Boolean(segment.allowCustom) ||
								(segment.options !== undefined &&
									segment.options.length > 0),
							{
								message:
									"identity segments must define options or set allowCustom: true",
								path: ["options"],
							},
						),
				)
...
export function parseConfig(raw: string, filePath: string): DevWizardConfig {
	...
	const config = rawConfig as DevWizardConfig;
...
function validateConfigIntegrity(config: DevWizardConfig, filePath: string) {
	const scenarioIds = new Set<string>();
	for (const scenario of config.scenarios) {
		if (scenarioIds.has(scenario.id)) {
			throw new Error(
				`Duplicate scenario id "${scenario.id}" detected in ${filePath}.`,
			);
		}
		...
		if (!config.flows[scenario.flow]) {
			throw new Error(
				`Scenario "${scenario.id}" references missing flow "${scenario.flow}" in ${filePath}.`,
			);
		}
		...
		if (scenario.flows) {
			for (const chainedFlow of scenario.flows) {
				if (!config.flows[chainedFlow]) {
					throw new Error(
						`Scenario "${scenario.id}" references missing flow "${chainedFlow}" in flows array (${filePath}).`,
					);
				}
			}
		}
	...
	for (const [flowId, flow] of Object.entries(config.flows)) {
		if (flow.id !== flowId) {
			throw new Error(
				`Flow key "${flowId}" does not match its id "${flow.id}" in ${filePath}.`,
			);
		}
		...
		for (const step of flow.steps) {
			if (stepIds.has(step.id)) {
				throw new Error(
					`Duplicate step id "${step.id}" found in flow "${flowId}" (${filePath}).`,
				);
			}
			stepIds.add(step.id);
		}
