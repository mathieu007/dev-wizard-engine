import type { PromptStep } from "../loader/types";

export function parseOverrideValue(step: PromptStep, rawValue: unknown): unknown {
	switch (step.mode) {
		case "confirm": {
			...
			throw new Error(
				`Override value for prompt "${step.id}" must be a boolean or boolean-like string.`,
			);
		}
		case "select": {
			const value =
			...
			if (typeof value !== "string") {
				throw new Error(
					`Override value for prompt "${step.id}" must be a string matching an available option.`,
				);
			}
			if (step.options && !step.options.some((option) => option.value === value)) {
				throw new Error(
					`Override value "${value}" is not a valid option for prompt "${step.id}".`,
				);
			}
			return value;
		}
		case "multiselect": {
			if (Array.isArray(rawValue)) {
				...
				if (step.options) {
					const invalid = values.filter(
						(value) => !step.options!.some((option) => option.value === value),
					);
					if (invalid.length > 0) {
						throw new Error(
							`Override values "${invalid.join(
								", ",
							)}" are not valid options for prompt "${step.id}".`,
						);
					}
				}
				return values;
			...
			throw new Error(
				`Override value for prompt "${step.id}" must be an array of strings or a comma-separated string.`,
			);
		}
		case "input":
		default: {
			...
			throw new Error(
				`Override value for prompt "${step.id}" must be a primitive string/number/boolean.`,
			);
		}
	}
}

export function validatePromptValue(step: PromptStep, value: unknown): void {
