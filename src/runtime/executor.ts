...
import type { WizardLogWriter, WizardLogEvent } from "./logWriter.js";
...
import type {
	...
	WizardPluginStepRegistration,
} from "./plugins.js";
import {
	createPromptHistoryManager,
	type PromptHistoryManager,
} from "./promptHistory.js";
import {
	PromptPersistenceManager,
	type PromptPersistenceTarget,
} from "./promptPersistence.js";
...
type ShortcutAction = "skip-step" | "replay-command" | "safe-abort";

interface ShortcutConfig<Value> {
	key: string;
	value: Value;
...
function logInfo(context: ExecutorContext, message: string): void {
	if (context.quiet) {
		return;
	}
...
function logSuccess(context: ExecutorContext, message: string): void {
	if (context.quiet) {
		return;
	}
	writeLine(context.stdout, message);
...
function logWarn(context: ExecutorContext, message: string): void {
	writeLine(context.stderr, message);
}

function logError(context: ExecutorContext, message: string): void {
	writeLine(context.stderr, message);
}

function logNote(context: ExecutorContext, title: string, body: string): void {
...
type WorktreeStatus = "clean" | "dirty";
type WorktreeStrategy = "commit-push" | "stash" | "branch" | "proceed";

const DEFAULT_WORKTREE_PROMPT =
	"Select a backup strategy before running the selected workflows.";
const DEFAULT_WORKTREE_CLEAN_MESSAGE =
	"Working tree is clean. Continuing.";
const DEFAULT_WORKTREE_DIRTY_MESSAGE =
	"Working tree has uncommitted changes. Choose how you want to back it up before automation runs.";
const DEFAULT_COMMIT_MESSAGE_PROMPT =
	"Commit message for the staged cleanup";
const DEFAULT_COMMIT_MESSAGE_TEMPLATE = "dev-wizard automation cleanup";
const DEFAULT_STASH_MESSAGE_PROMPT =
	"Stash message describing these saved changes";
const DEFAULT_STASH_MESSAGE_TEMPLATE = "dev-wizard automation stash";
const DEFAULT_BRANCH_PROMPT =
	"Branch name for this automation snapshot";
...
export async function executeScenario(
	...
	const scenario = context.config.scenarios.find(
	...
	const state: WizardState = initialState
	...
	try {
		...
		for (let index = startingFlowIndex; index < flowSequence.length; index += 1) {
			...
			state.flowRuns.push({
				...
				durationMs: Math.max(
					0,
					flowEndedAt.getTime() - flowStartedAt.getTime(),
				),
		...
		if (!context.dryRun) {
				try {
					...
					logWarn(
						context,
						`Failed to write workflow analytics: ${error instanceof Error ? error.message : String(error)}`,
					);
				}
			}
		emitLog(context, {
...
function ensurePromptHistory(context: ExecutorContext): PromptHistoryManager {
...
export async function buildScenarioPlan(
	...
	const clonedOverrides =
		context.overrides && Object.keys(context.overrides).length > 0
			? structuredClone(context.overrides)
			: {};

	const planContext: ExecutorContext = {
		...context,
	...
	state.flowCursor = clampIndex(
		state.flowCursor ?? 0,
		buildScenarioFlowSequence(scenario, context.phase).length,
	);
	...
	for (let index = startingFlowIndex; index < flowSequence.length; index += 1) {
		const flowId = flowSequence[index]!;
		const startingStepIndex =
			index === startingFlowIndex ? clampIndex(state.stepCursor ?? 0, context.config.flows[flowId]?.steps.length ?? 0) : 0;
...
async function previewFlow(
...
): Promise<{ flowPlan: FlowPlan; exitTarget?: StepTransitionTarget }> {
	...
	for (
		let pointer = clampIndex(startingStepIndex, flow.steps.length);
		pointer < flow.steps.length;
		pointer += 1
	) {
		const step = flow.steps[pointer];
		const templateContext = buildTemplateContext(context, state, step);
		const preview = await previewStep(
			flow,
			step,
			state,
			context,
		...
		if (typeof preview.next === "string") {
			...
			if (typeof nextIndex !== "number") {
				plan.warnings.push(
					`Step "${step.id}" attempted to jump to unknown step "${preview.next}" during plan preview.`,
				);
				break;
			}
			pointer = nextIndex - 1;
...
async function previewStep(
	flow: DevWizardFlow,
	step: DevWizardFlow["steps"][number],
	state: WizardState,
	context: ExecutorContext,
...
function collectPlanOverrides(overrides: Record<string, unknown>): PlanOverride[] {
...
function buildPluginPlan(
	step: PluginStep,
	pluginName: string,
	result?: WizardPluginPlanResult,
): PluginStepPlan {
	const basePlan: PluginStepPlan = {
		kind: "plugin",
		id: step.id,
		label: step.label,
...
function normalizePluginStepResult(
	result?: WizardPluginStepResult,
): StepResult {
	return {
		next: result?.next,
...
function getPluginRegistration(
	context: ExecutorContext,
	stepType: string,
): WizardPluginStepRegistration {
	...
	if (!registration) {
		throw new Error(
			`Unsupported step type "${stepType}". Register a plugin that provides this step type.`,
		);
	}
	return registration;
}

function ensurePluginRegistry(context: ExecutorContext): WizardPluginRegistry {
	if (!context.plugins) {
...
async function previewCommandStep(
	flowId: string,
	step: CommandStep,
	state: WizardState,
	context: ExecutorContext,
	templateContext: TemplateContext,
	plan: ScenarioPlan,
): Promise<{ plan: CommandStepPlan; next?: StepTransitionTarget }> {
	const commandPlans: CommandPreview[] = [];

	for (let index = 0; index < step.commands.length; index += 1) {
		const rawDescriptor = step.commands[index]!;
		const resolvedDescriptor = resolveCommandDescriptor(
			rawDescriptor,
			step,
			context.config,
		);
		const renderedDescriptor = renderMaybeNested(
			resolvedDescriptor,
			templateContext,
		) as CommandDescriptor;

		const commandPresetName =
			rawDescriptor.preset ?? step.defaults?.preset ?? resolvedDescriptor.preset;
		const commandPreset = commandPresetName
			? getResolvedCommandPreset(context.config, commandPresetName)?.definition
			: undefined;

		const envDiff = computeEnvDiffEntries(
			commandPreset?.env ?? {},
			step.defaults?.env ?? {},
			rawDescriptor.env ?? {},
		);
		...
		commandPlans.push({
			...
			warnAfterMs,
			continueOnFail: Boolean(
				rawDescriptor.continueOnFail ?? step.continueOnError ?? false,
			),
	...
	const planStep: CommandStepPlan = {
		kind: "command",
		id: step.id,
		label: step.label,
...
function previewMessageStep(
	...
	const planStep: MessageStepPlan = {
		kind: "message",
		id: step.id,
		label: step.label,
		level: step.level,
...
async function previewPromptStep(
	flowId: string,
	step: PromptStep,
	state: WizardState,
	context: ExecutorContext,
	...
	let answerSource: PromptStepPlan["answerSource"] = "pending";
	let answer: unknown;
	...
	if (answerSource === "pending") {
		...
		plan.warnings.push(
			`Prompt "${step.id}" requires input at runtime.`,
		);
	}

	let options: PromptPlanOption[] | undefined;
	let dynamic = false;
	if (expectsOptions) {
...
function previewBranchStep(
	flowId: string,
	step: BranchStep,
	state: WizardState,
	templateContext: TemplateContext,
	plan: ScenarioPlan,
): { plan: BranchStepPlan; next?: StepTransitionTarget } {
	const branches: BranchPreview[] = [];
	let selectedTarget: StepTransitionTarget | undefined;
	...
	for (const branch of step.branches) {
		...
		const branchPreview: BranchPreview = {
			expression,
			result,
			target: branch.next,
	...
	if (finalTarget === undefined) {
		plan.warnings.push(
			`Branch step "${step.id}" does not select a target during preview.`,
		);
	}

	const planStep: BranchStepPlan = {
		kind: "branch",
		id: step.id,
		label: step.label,
		branches,
...
async function previewIterateStep(
	flowId: string,
	step: IterateStep,
	state: WizardState,
	context: ExecutorContext,
	...
	if (Array.isArray(step.items)) {
		...
		switch (step.source.from) {
			...
			case "json": {
				...
				try {
					const options =
						(await resolveDynamicPromptOptions(
							{
								type: "json",
	...
	const planStep: IterateStepPlan = {
		kind: "iterate",
		id: step.id,
		label: step.label,
		flowId: step.flow,
...
async function previewComputeStep(
	flowId: string,
	step: ComputeStep,
	state: WizardState,
	templateContext: TemplateContext,
...
async function previewGitWorktreeGuardStep(
	flowId: string,
	...
	templateContext: TemplateContext,
	plan: ScenarioPlan,
): Promise<{ plan: GitWorktreeGuardPlan }> {
	...
	const strategy =
		readStoredStrategy(state.answers[strategyKey]) ??
	...
	const planStep: GitWorktreeGuardPlan = {
		kind: "git-worktree-guard",
		id: step.id,
		label: step.label,
		status,
...
async function previewPluginStep(
	flow: DevWizardFlow,
	step: PluginStep,
	state: WizardState,
	context: ExecutorContext,
...
function computeEnvDiffEntries(
	...
	for (const key of keys) {
		if (Object.prototype.hasOwnProperty.call(commandEnv, key)) {
			const previous =
				Object.prototype.hasOwnProperty.call(defaultEnv, key)
					? defaultEnv[key]
					: presetEnv[key];
			entries.push({
				key,
...
async function runFlow(
...
): Promise<StepTransitionTarget | undefined> {
	...
	for (
		let pointer = clampIndex(options.startingStepIndex ?? 0, flow.steps.length);
		pointer < flow.steps.length;
		pointer += 1
	) {
		const step = flow.steps[pointer];
		...
		if (typeof nextTarget === "string") {
			...
			if (typeof nextIndex !== "number") {
				throw new Error(
					`Step "${step.id}" attempted to jump to unknown step "${nextTarget}".`,
				);
			}
...
async function runPostRunHooks(
...
interface RenderedPromptOption extends PromptOption {
	fullLabel: string;
	fullHint?: string;
	isLabelTruncated: boolean;
	isHintTruncated: boolean;
}
...
function determineMaxVisiblePromptItems(optionCount: number): number | undefined {
	if (optionCount <= 0) {
		return undefined;
	}
...
function logFullPromptOptions(
	context: ExecutorContext,
	step: PromptStep,
	options?: RenderedPromptOption[],
): void {
	void context;
	void step;
	void options;
...
function normalizeMultiselectDefault(value: unknown): string[] | undefined {
...
async function executePromptStep(
		...
		const persisted = context.promptPersistence.get(persistenceTarget);
			...
			logPromptPersistenceEvent(
				...
				if (applied) {
					...
					logSuccess(
						context,
						`[persisted] ${key}=${formatOverrideDisplay(persisted, step.mode)}`,
					);
					emitLog(context, {
						type: "prompt.answer",
						flowId,
	...
	let promptOptions: RenderedPromptOption[] | undefined;
	const expectsOptions = step.mode === "select" || step.mode === "multiselect";
	if (expectsOptions) {
		promptOptions = await buildPromptOptions(step, templateContext, context);
		if (promptOptions.length === 0) {
			throw new Error(
				`Prompt "${step.id}" did not produce any options. Ensure static options or dynamic sources are configured.`,
			);
		}
	}
	...
	if (context.nonInteractive) {
		throw new Error(
			`Non-interactive run cannot prompt for "${step.id}" (${key}). Provide it via --answers/--set or run in collect mode first.`,
		);
	}
		...
		case "multiselect":
			{
				...
				const optionsList = (promptOptions ?? []) as PromptOption[];
				const maxVisibleItems = determineMaxVisiblePromptItems(
...
function resolvePromptPersistenceTarget(
	step: PromptStep,
	state: WizardState,
): PromptPersistenceTarget | undefined {
...
function getCurrentProjectId(state: WizardState): string | undefined {
	const project = state.answers.project;
	if (project && typeof project === "object") {
		const candidate = (project as Record<string, unknown>).id;
...
function persistPromptAnswer(
	context: ExecutorContext,
	target: PromptPersistenceTarget | undefined,
	value: unknown,
) {
...
function logPromptPersistenceEvent(
	context: ExecutorContext,
	flowId: string,
	stepId: string,
	target: PromptPersistenceTarget,
	status: "hit" | "miss",
	applied?: boolean,
): void {
	emitLog(context, {
...
function renderPromptOption(
	...
	const truncatedLabelText = truncateText(
		renderedLabel,
		PROMPT_LABEL_MAX_LENGTH,
	);
	const truncatedHintText = truncateText(
		renderedHint,
		PROMPT_HINT_MAX_LENGTH,
	);
	...
	return {
		...
		isLabelTruncated:
			renderedLabel !== undefined && (truncatedLabelText ?? renderedLabel) !== renderedLabel,
		isHintTruncated:
			renderedHint !== undefined && (truncatedHintText ?? renderedHint) !== renderedHint,
	} satisfies RenderedPromptOption;
}
...
async function buildPromptOptions(
	...
	templateContext: TemplateContext,
	context: ExecutorContext,
): Promise<RenderedPromptOption[]> {
	assertDynamicCommandAllowed(step.id, step.dynamic, context, "prompt");
	const dynamicContext = getDynamicContext(context);

	const dynamicOptions =
		(await resolveDynamicPromptOptions(step.dynamic, templateContext, dynamicContext)) ??
		[];

	const combined = [...dynamicOptions, ...(step.options ?? [])];
	const rendered = combined.map((option) =>
		renderPromptOption(option, templateContext),
	);

	const unique = new Map<string, RenderedPromptOption>();
...
async function resolveIterationItems(
	step: IterateStep,
	state: WizardState,
	templateContext: TemplateContext,
	context: ExecutorContext,
	...
	if (step.items === undefined && step.source === undefined) {
		throw new Error(
			`Iterate step "${step.id}" must define either items or a source.`,
		);
	}
...
function assertDynamicCommandAllowed(
	stepId: string,
	dynamic: PromptDynamicOptions | undefined,
	context: ExecutorContext,
	sourceLabel: string,
): void {
	...
	throw new Error(
		`Collect mode cannot resolve dynamic.command options for ${sourceLabel} "${stepId}". Use "glob", "json", or "workspace-projects" providers, or move the command into a compute step.`,
	);
}
...
function getDynamicContext(context: ExecutorContext): DynamicOptionsContext {
	const cache = context.promptOptionsCache ?? new Map<string, PromptOption[]>();
...
function maybeStoreCommandOutput(
	state: WizardState,
	...
	const parseOptions =
		typeof descriptor.parseJson === "object"
	...
	if (descriptor.parseJson) {
		try {
			...
			if (parseOptions?.onError === "warn") {
				logWarn(
					context,
					`Failed to parse JSON output for ${chalk.cyan(record.stepId)}: ${
						error instanceof Error ? error.message : String(error)
					}. Using raw stdout instead.`,
				);
				value = rawOutput;
			} else {
				throw new Error(
					`Failed to parse JSON output for ${record.stepId}: ${
						error instanceof Error ? error.message : String(error)
					}`,
				);
			}
	...
	function createReviver(
		context: ExecutorContext,
		code: string,
	): ((this: unknown, key: string, value: unknown) => unknown) | undefined {
		if (!code.trim()) {
			return undefined;
		}
		...
		try {
			...
			const fn = new Function("key", "value", code) as (
				key: string,
				value: unknown,
			) => unknown;
			...
			logWarn(
				context,
				`Ignoring invalid parseJson.reviver script: ${
					error instanceof Error ? error.message : String(error)
				}.`,
			);
			return undefined;
...
function applyRedactions(value: unknown, paths: string[]): unknown {
...
function getSegment(segment: string): string | number {
	if (/^\d+$/.test(segment)) {
...
function resolveCommandDescriptor(
	...
	if (presetName && !preset) {
		throw new Error(
			`Command preset "${presetName}" referenced by step "${step.id}" was not found in commandPresets.`,
		);
	}
...
function withoutPresetMetadata(
	preset: Readonly<CommandPreset>,
): CommandDefaults {
...
async function executeIterateStep(
	flowId: string,
	step: IterateStep,
	state: WizardState,
	context: ExecutorContext,
	...
	if (items.length === 0) {
		logWarn(
			context,
			`[iterate] Step ${chalk.cyan(step.id)} did not resolve any items. Skipping.`,
		);
	...
	if (step.concurrency && step.concurrency > 1) {
		logWarn(
			context,
			`[iterate] concurrency > 1 is not yet supported (step ${chalk.cyan(step.id)}). Falling back to sequential execution.`,
		);
	}
...
async function resolveComputeStepValues(
	step: ComputeStep,
	state: WizardState,
	templateContext: TemplateContext,
	context: ExecutorContext,
): Promise<{ values: Record<string, unknown>; handler?: string; storeAs?: string }> {
	if (step.handler) {
...
function normalizeComputeParams(
	step: ComputeStep,
	params: unknown,
): Record<string, unknown> {
	if (!params || typeof params !== "object" || Array.isArray(params)) {
		throw new Error(
			`Compute step "${step.id}" params must resolve to an object.`,
		);
	}
	return params as Record<string, unknown>;
}

function normalizeComputeResult(
	step: ComputeStep,
	result: unknown,
): Record<string, unknown> {
	if (step.storeAs) {
	...
	throw new Error(
		`Compute step "${step.id}" must define storeAs when the handler result is not an object.`,
	);
}

async function executeGitWorktreeGuardStep(
	flowId: string,
	step: GitWorktreeGuardStep,
	state: WizardState,
	context: ExecutorContext,
	...
	if (status === "clean") {
		const message = renderTemplate(
			step.cleanMessage ?? DEFAULT_WORKTREE_CLEAN_MESSAGE,
			templateContext,
		);
		if (!context.quiet) {
	...
	const dirtyMessage = renderTemplate(
		step.dirtyMessage ?? DEFAULT_WORKTREE_DIRTY_MESSAGE,
		templateContext,
	);
	if (!context.quiet) {
	...
	const strategy = await ensureWorktreeStrategy(
		flowId,
		step,
		state,
		context,
	...
	if (strategy === "commit-push" || strategy === "branch") {
		...
		if (strategy === "branch") {
			branchName = await ensureBranchName(
				flowId,
				step,
				state,
				context,
		...
		const commitMessage = await ensureCommitMessage(
			flowId,
			step,
			state,
			context,
		...
		if (!context.quiet) {
			...
			logSuccess(
				context,
				`[git] Committed and pushed local changes${branchSuffix} (${commitMessage}).`,
			);
		}
	} else if (strategy === "stash") {
		const stashMessage = await ensureStashMessage(
			flowId,
			step,
			state,
			context,
...
function resolveWorktreeGuardCwd(
	step: GitWorktreeGuardStep,
	templateContext: TemplateContext,
	context: ExecutorContext,
): string {
...
async function readWorktreeStatus(cwd: string): Promise<WorktreeStatus> {
	try {
		...
		throw new Error(
			`Failed to inspect the git working tree in ${cwd}: ${error instanceof Error ? error.message : String(error)}`,
		);
	}
}

function getStrategyAnswerKey(step: GitWorktreeGuardStep): string {
	return step.storeStrategyAs ?? `${step.id}Strategy`;
}

function getCommitMessageKey(step: GitWorktreeGuardStep): string {
	return step.storeCommitMessageAs ?? `${step.id}CommitMessage`;
}

function getStashMessageKey(step: GitWorktreeGuardStep): string {
	return step.storeStashMessageAs ?? `${step.id}StashMessage`;
}

function getBranchNameKey(step: GitWorktreeGuardStep): string {
	return step.storeBranchNameAs ?? `${step.id}BranchName`;
}

function readStoredStrategy(value: unknown): WorktreeStrategy | undefined {
	if (typeof value !== "string") {
		return undefined;
	}
	if (
...
function peekStrategyOverride(
	step: GitWorktreeGuardStep,
	context: ExecutorContext,
	state: WizardState,
	key: string,
): WorktreeStrategy | undefined {
	const override = getOverride(context, key, step.id);
	if (override === undefined) {
		return undefined;
	}
	const normalized = readStoredStrategy(
		typeof override === "string" ? override : String(override),
	);
	if (!normalized) {
		throw new Error(
			`Override for "${key}" must be one of: commit-push, stash, branch, proceed.`,
		);
	}
...
function buildDirtyWorktreePreviewMessage(
	step: GitWorktreeGuardStep,
	templateContext: TemplateContext,
	strategy: WorktreeStrategy | undefined,
	state: WizardState,
): string {
	if (strategy) {
		const meta = STRATEGY_METADATA[strategy];
		if (strategy === "branch") {
			const branchName = readStoredString(
				state.answers[getBranchNameKey(step)],
			);
			if (branchName) {
...
async function ensureWorktreeStrategy(
	flowId: string,
	step: GitWorktreeGuardStep,
	state: WizardState,
	context: ExecutorContext,
...
async function ensureCommitMessage(
	...
	const prompt: PromptStep = {
		...
		prompt: renderTemplate(
			step.commitMessagePrompt ?? DEFAULT_COMMIT_MESSAGE_PROMPT,
			templateContext,
		),
		defaultValue: renderTemplate(
			step.commitMessageDefault ?? DEFAULT_COMMIT_MESSAGE_TEMPLATE,
			templateContext,
		),
	};
...
async function ensureStashMessage(
	flowId: string,
	step: GitWorktreeGuardStep,
	state: WizardState,
	context: ExecutorContext,
	...
	const prompt: PromptStep = {
		...
		required: true,
		prompt: renderTemplate(
			step.stashMessagePrompt ?? DEFAULT_STASH_MESSAGE_PROMPT,
			templateContext,
		),
		defaultValue: renderTemplate(
			step.stashMessageDefault ?? DEFAULT_STASH_MESSAGE_TEMPLATE,
			templateContext,
		),
	};
...
async function ensureBranchName(
	flowId: string,
	step: GitWorktreeGuardStep,
	state: WizardState,
	context: ExecutorContext,
	...
	const prompt: PromptStep = {
		...
		required: true,
		prompt: renderTemplate(
			step.branchNamePrompt ?? DEFAULT_BRANCH_PROMPT,
			templateContext,
		),
		defaultValue,
	};
...
async function applyCommitStrategy(cwd: string, message: string): Promise<void> {
	...
	try {
		...
		throw new Error(
			`Failed to commit and push local changes: ${error instanceof Error ? error.message : String(error)}`,
		);
	}
}

async function applyBranchStrategy(cwd: string, branchName: string): Promise<void> {
	try {
...
async function applyStashStrategy(cwd: string, message: string): Promise<void> {
	try {
		...
		throw new Error(
			`Failed to stash local changes: ${error instanceof Error ? error.message : String(error)}`,
		);
	}
}

function buildStrategyOptions(step: GitWorktreeGuardStep): PromptOption[] {
	const allowed = listAllowedStrategies(step);
...
async function autoCommitNestedRepositories(cwd: string, message: string): Promise<void> {
	const manifestPath = path.join(cwd, "workspace.repos.json");
	if (!(await pathExists(manifestPath))) {
		return;
	}

	let manifest: Array<{ path?: string | null }> = [];
	try {
	...
	for (const entry of manifest) {
		...
		try {
			...
			throw new Error(
				`Failed to commit nested repository "${repoPath}": ${text}`,
			);
		}
		...
		try {
			...
			const text = error instanceof Error ? error.message : String(error);
			throw new Error(
				`Failed to push nested repository "${repoPath}": ${text}`,
			);
		}
	}
}
...
async function getDefaultRemoteName(cwd: string, branch: string): Promise<string> {
	try {
...
async function pushWithUpstreamFallback(cwd: string): Promise<void> {
	try {
...
function listAllowedStrategies(step: GitWorktreeGuardStep): WorktreeStrategy[] {
	const options: WorktreeStrategy[] = [];
	...
	if (options.length === 0) {
		throw new Error(
			`Git worktree guard step "${step.id}" does not enable any strategies.`,
		);
	}
	return options;
}
...
const STRATEGY_METADATA: Record<
	WorktreeStrategy,
	{ label: string; hint: string; planSummary: string }
> = {
	"commit-push": {
	...
	branch: {
		...
		planSummary:
			"Working tree is dirty. Will create a new branch, then commit and push local changes before automation runs.",
	},
	proceed: {
...
function normalizeWarnAfterMs(value: unknown): number | undefined {
	if (value === undefined || value === null) {
		return undefined;
	}
...
async function executeCommandStep(
	...
	if (context.phase === "collect" && !step.collectSafe) {
		throw new Error(
			`Collect mode reached command step "${step.id}" in flow "${flowId}". Mark it collectSafe or replace it with a compute step.`,
		);
	}
...
function canPromptForPolicy(context: ExecutorContext): boolean {
	const stdout = context.stdout as { isTTY?: boolean } | undefined;
	if (context.quiet) {
		return false;
	}
...
interface RunCommandInput {
	flowId: string;
	step: CommandStep;
	rawDescriptor: CommandDescriptor;
	renderedDescriptor: CommandDescriptor;
	context: ExecutorContext;
}

interface EnforceCommandPolicyInput {
	context: ExecutorContext;
	flowId: string;
	step: CommandStep;
	rawDescriptor: CommandDescriptor;
...
async function enforceCommandPolicy({
	context,
	flowId,
	step,
	rawDescriptor,
	renderedDescriptor,
}: EnforceCommandPolicyInput): Promise<void> {
	...
	let decision: PolicyDecision = initialDecision;
	...
	const warnMessage = () => {
		...
		logWarn(
			context,
			`${chalk.yellow("[policy]")} ${rule.id}${acknowledgement}: ${renderedDescriptor.run}${note}`,
		);
	};
		...
		if (decision.enforcedLevel === "block") {
			...
			if (!canPromptForPolicy(context)) {
				...
				logPolicyDecision();
				throw new Error(
					`Command "${renderedDescriptor.run}" blocked by policy "${rule.id}". Use --policy-ack ${rule.id} to proceed.`,
				);
			}
			...
			if (proceed === false) {
				logPolicyDecision();
				throw new Error(
					`Command "${renderedDescriptor.run}" blocked by policy "${rule.id}".`,
				);
			}

		policyEngine.acknowledge(rule.id);
		const reevaluatedDecision =
			policyEngine.evaluateCommand(evaluationInput) ?? decision;
		decision = reevaluatedDecision;
			logWarn(
				context,
				`${chalk.yellow("[policy acknowledged]")} ${rule.id}: proceeding after acknowledgement.`,
			);
		}
	...
	async function runCommand({
		...
		renderedDescriptor,
		context,
	}: RunCommandInput): Promise<CommandExecutionRecord> {
	const startedAt = new Date();
	const quietCommand = Boolean(renderedDescriptor.quiet);
	const requiresStdoutCapture = Boolean(
		renderedDescriptor.captureStdout ||
		quietCommand ||
		renderedDescriptor.storeStdoutAs ||
		renderedDescriptor.parseJson,
	);
	const stdoutMode = requiresStdoutCapture ? "pipe" : "inherit";
		const warnAfterMs = normalizeWarnAfterMs(
			(renderedDescriptor as { warnAfterMs?: unknown }).warnAfterMs,
		);
		...
		if (context.verbose && !context.quiet && (!context.dryRun || executingInDryRun)) {
			const details: string[] = [renderedDescriptor.run];
		...
		const shouldSkipExecution =
			registerMode || (!allowCollectExecution && context.dryRun && !executingInDryRun);
		if (shouldSkipExecution) {
			const label = collectMode
				...
				logInfo(
					context,
					`${chalk.gray(label)} ${renderedDescriptor.run}${renderedDescriptor.cwd ? chalk.gray(` (cwd: ${renderedDescriptor.cwd})`) : ""}`,
				);
		return {
			flowId,
			stepId: step.id,
		...
		if (executingInDryRun) {
			logInfo(
				context,
				`${chalk.gray("[dry-run execute]")} ${renderedDescriptor.run}${renderedDescriptor.cwd ? chalk.gray(` (cwd: ${renderedDescriptor.cwd})`) : ""
				}`,
			);
		}
	...
	try {
		...
		const longRunning =
			typeof warnAfterMs === "number" && durationMs >= warnAfterMs;

			if (longRunning && !context.quiet) {
				logWarn(
					context,
					`${chalk.yellow("⚠")} ${renderedDescriptor.run} exceeded ${formatDurationForLog(
						warnAfterMs!,
					)} (took ${formatDurationForLog(durationMs)})`,
				);
			}
			...
			if (context.verbose && !context.quiet) {
				logSuccess(
					context,
					`${chalk.gray("✓")} ${renderedDescriptor.run} (${formatDurationForLog(durationMs)})`,
				);
			}
		...
		const longRunning =
			...
			logError(
				context,
				`Command "${renderedDescriptor.run}" failed${typeof failure.exitCode === "number"
			...
			if (timedOut && !context.quiet) {
				logWarn(
					context,
					`${chalk.yellow("⚠")} ${renderedDescriptor.run} timed out after ${formatDurationForLog(
						renderedDescriptor.timeoutMs ?? durationMs,
					)}.`,
			);
		}
...
async function handleCommandFailure(
	...
	if (autoHandled) {
		...
		if (policyResolution && "target" in policyResolution) {
			...
			logAutoAction(
				context,
				step,
				`policy "${policyResolution.key}" -> ${policyResolution.target}`,
			);
			return {
		...
		if (
			...
			throw new Error(
				`Missing policy "${policyResolution.key}" for command "${step.id}". Provide it via answers before executing unattended runs.`,
			);
		}

		const actions = step.onError?.actions ?? [];
		const actionLabels = new Map<StepTransitionTarget, string>();
		...
		try {
			selection = supportsShortcuts
				? await context.promptDriver.selectWithShortcuts({
						...
						] satisfies ShortcutConfig<string>[],
						onShortcut(action) {
	...
	switch (selection) {
		...
		default: {
			const target = selection as StepTransitionTarget;
...
function supportsShortcutPrompts(context: ExecutorContext): boolean {
	if (context.quiet) {
		return false;
	}
	const stdout = context.stdout as { isTTY?: boolean } | undefined;
...
function recordShortcutTrigger({
	context,
	step,
	flowId,
	action,
...
function recordRetry(state: WizardState, flowId: string, step: CommandStep): void {
	state.retries.push({
		flowId,
		stepId: step.id,
		stepLabel: step.label ?? step.id,
...
function resolveOnErrorPolicy(
	policy: CommandErrorPolicy | undefined,
	state: WizardState,
): { target: StepTransitionTarget; key: string } | { missing: true; key: string; required: boolean } | undefined {
	if (!policy) {
		return undefined;
	}
...
function maybeApplyAutoHandling(
	flowId: string,
	step: CommandStep,
	state: WizardState,
	context: ExecutorContext,
...
function findActionLabel(
	actions: TransitionAction[] | undefined,
	target: StepTransitionTarget | undefined,
): string | undefined {
	if (!actions || !target) {
		return undefined;
	}
...
function buildTemplateContext(
	...
	const identityById = identity
		? identity.segments.reduce<Record<string, WizardIdentitySegmentSelection>>((acc, segment) => {
...
function chainLogWriters(
	writers: (WizardLogWriter | undefined)[],
): WizardLogWriter | undefined {
	const filtered = writers.filter(Boolean) as WizardLogWriter[];
...
function emitLog(context: ExecutorContext, event: WizardLogEvent): void {
...
function reportStepCompletion(
	context: ExecutorContext,
	breadcrumb: string,
	status: StepResult["status"],
	nextTarget: StepTransitionTarget | undefined,
	dryRun: boolean,
	quiet: boolean,
	durationMs: number,
): void {
	...
	const suffix =
		nextTarget && nextTarget !== "exit" && nextTarget !== "repeat"
			? ` → ${nextTarget}`
			: nextTarget === "repeat"
				? " (retrying)"
				: "";
	const durationLabel =
		context.verbose && !Number.isNaN(durationMs)
...
function recordIntegrationTiming(record: CommandExecutionRecord, state: WizardState): void {
	if (!record.stdout) {
		return;
	}
...
function buildScenarioFlowSequence(
	scenario: DevWizardScenario,
	phase?: ExecutorContext["phase"],
): string[] {
	if (phase === "collect") {
		return [scenario.flow];
	}
	const sequence: string[] = [scenario.flow];
...
function getScenarioDuration(state: WizardState): number {
	const end = state.endedAt ?? new Date();
	return Math.max(0, end.getTime() - state.startedAt.getTime());
}

function createInitialState(scenario: DevWizardScenario): WizardState {
	return {
		scenario,
		answers: {},
		history: [],
...
function prepareResumeState(state: WizardState, scenario: DevWizardScenario): WizardState {
	state.scenario = scenario;
...
function removeOverride(context: ExecutorContext, key: string, stepId: string) {
...
function formatOverrideDisplay(value: unknown, mode: PromptStep["mode"]): string {
	if (Array.isArray(value)) {
		return value.join(", ");
	}
