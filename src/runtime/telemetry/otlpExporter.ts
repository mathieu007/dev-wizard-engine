...
interface TraceContext {
	...
	spans: OtlpSpan[];
	activeSteps: Map<string, ActiveSpan>;
	resourceAttributes: Record<string, string>;
	scopeName: string;
}
...
export function createOtlpLogWriter(options: OtlpExporterOptions): WizardLogWriter {
	const headers = {
	...
	return {
		write(event) {
			switch (event.type) {
				...
				case "step.complete": {
					...
					const startTimeMs =
						active?.startTimeMs ?? endTimeMs - (event.durationMs ?? 0);
					const span: OtlpSpan = {
						...
						endTimeUnixNano: toUnixNano(
							active ? active.startTimeMs + (event.durationMs ?? 0) : endTimeMs,
						),
						status: {
						...
						attributes: [
							...
							makeAttribute("dev_wizard.step_index", String(event.index)),
							makeAttribute(
								"dev_wizard.step_next",
								event.next ? String(event.next) : "null",
							),
						],
				...
				case "command.result": {
					...
					if (typeof event.durationMs === "number") {
						attributes.push(
							makeAttribute("dev_wizard.duration_ms", String(event.durationMs)),
						);
					}
		...
		case "prompt.persistence": {
			...
			if (event.projectId) {
				attributes.push(
					makeAttribute("dev_wizard.project_id", event.projectId),
				);
			}
			if (typeof event.applied === "boolean") {
				attributes.push(
					makeAttribute("dev_wizard.persistence_applied", String(event.applied)),
				);
			}
			spanState.events.push({
...
function makeAttribute(key: string, value: string): OtlpKeyValue {
	return {
		key,
		value: {
			stringValue: value,
...
function mergeAttributes(base: OtlpKeyValue[], extra: OtlpKeyValue[]): OtlpKeyValue[] {
...
async function sendTrace(
	...
	const resourceAttributes = Object.entries(context.resourceAttributes).map(
		([key, value]) => makeAttribute(key, value),
	);
