...
const computeHandlers = new Map();
export function registerComputeHandler(id, handler) {
    if (computeHandlers.has(id)) {
        throw new Error(`Compute handler "${id}" is already registered.`);
    }
    computeHandlers.set(id, handler);
}
export function getComputeHandler(id) {
...
registerComputeHandler("workspace-projects", async (params, context) => {
    ...
    const selectedProjects = readStringArray(params.selectedProjects) ??
...
registerComputeHandler("render-typecheck-command", async (params, context) => {
    ...
    const scriptPath = path.join(workspaceRoot, "packages/dev-wizard-presets/scripts/typecheck.ts");
...
function parseTemplateJsonValue(value) {
    if (typeof value === "string") {
        ...
        try {
            ...
            throw new Error(`Compute handler "template-json" failed to parse JSON: ${message}`);
        }
    }
    return value;
}
function normalizeMaintenanceCadence(value) {
...
function sanitizeMaintenanceWindowName(value) {
    const trimmed = value?.trim();
    if (!trimmed) {
        return undefined;
    }
...
function buildCommitMessageFallback(filePath) {
...
async function hasWorkspaceManifest(dir) {
...
//# sourceMappingURL=computeHandlers.js.map