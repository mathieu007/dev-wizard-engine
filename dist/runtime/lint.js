...
const LEGACY_LIBRARY_SCRIPT_PATTERN = /packages[\/\\-]dev-wizard-core[\/\\]examples[\/\\]library[\/\\]scripts[\/\\]/u;
export async function lintWizard(options = {}) {
    ...
    if (resolution.paths.length === 0) {
        issues.push({
            level: "error",
            message: "No Dev Wizard configuration files were found. Provide --config <path> or add dev-wizard.config.* / dev-wizard-config/index.*",
        });
        return {
            cwd,
            resolution,
...
function runSemanticChecks({ config, resolution, issues, cwd, }) {
...
async function validateAnswerArtifacts({ config, cwd, issues, answersPath, manifestPath, scenarioId, }) {
...
async function validateManifestAnswers({ config, cwd, issues, manifestPath, scenarioId, }) {
    ...
    const resolvedScenarioId = scenarioId ?? manifestScenarioId ?? (config.scenarios.length === 1 ? config.scenarios[0]?.id : undefined);
    if (!resolvedScenarioId) {
        issues.push({
            level: "error",
            message: "Unable to determine scenario for manifest validation. Provide --scenario or ensure the manifest includes scenarioId.",
            file,
        });
        return;
    }
...
async function validateAnswersFile({ config, cwd, issues, answersPath, scenarioId, }) {
    ...
    const resolvedScenarioId = scenarioId ?? payload.scenarioId ?? (config.scenarios.length === 1 ? config.scenarios[0]?.id : undefined);
    if (!resolvedScenarioId) {
        issues.push({
            level: "error",
            message: "Unable to determine scenario for answers validation. Provide --scenario or include meta.scenarioId in the answers file.",
            file,
        });
        return;
    }
...
function validateAnswersAgainstScenario({ config, scenarioId, answers, perProject, issues, file, }) {
    ...
    const flowGraph = buildFlowGraph(config);
    const startFlows = new Set([scenario.flow]);
        ...
        for (const [projectId, projectAnswers] of Object.entries(perProject)) {
            ...
            for (const { flowId, step } of promptSteps) {
                ...
                try {
                    ...
                    issues.push({
                        level: "error",
                        message: `Invalid answer for prompt "${step.id}" (key "${key}")` +
    ...
    for (const { flowId, step } of commandSteps) {
        ...
        if (!target) {
            ...
            issues.push({
                level: "error",
                message: `Policy value "${value}" for "${policy.key}" is not mapped in step "${step.id}" (flow "${flowId}").`,
...
function extractAnswersPayload(payload) {
    const meta = payload.meta;
    const scenarioId = meta && typeof meta === "object" && typeof meta.scenarioId === "string"
        ? String(meta.scenarioId)
        : undefined;
    if (Object.prototype.hasOwnProperty.call(payload, "scenario") ||
...
function resolveAnswerPath(answers, pathKey) {
    if (!pathKey.includes(".")) {
...
function isProjectScopedPrompt(step) {
...
function validateGroupStep(step, flowId, flow, flowIds, issues, cwd, resolution) {
...
function validateIterateStep(step, flowId, flowIds, issues, cwd, resolution) {
    if (!flowIds.has(step.flow)) {
        issues.push({
            level: "error",
...
function validatePromptStep(step, flowId, issues, cwd, resolution) {
    if ((step.mode === "select" || step.mode === "multiselect") &&
...
function validateBranchStep(step, flowId, stepIds, issues, cwd, resolution) {
...
function validateCommandStep(step, config, flowId, issues, cwd, resolution, usedPresets) {
    ...
    if (stepPresetName) {
        ...
        if (preset) {
            if (step.defaults?.shell !== undefined &&
            ...
            if (step.defaults?.cwd !== undefined &&
...
function validateGitWorktreeGuardStep(step, flowId, issues, cwd, resolution) {
    const options = {
...
export function formatLintResult(result, format) {
    if (format === "json") {
...
function buildFlowGraph(config) {
    const graph = new Map();
    for (const [flowId, flow] of Object.entries(config.flows)) {
        const edges = new Set();
        for (const step of flow.steps) {
...
function buildFlowStepIndex(config) {
    const map = new Map();
    for (const [flowId, flow] of Object.entries(config.flows)) {
        map.set(flowId, new Set(flow.steps.map((step) => step.id)));
    }
    return map;
}
function computeReachableFlows(initial, graph) {
