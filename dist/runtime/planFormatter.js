...
export function formatScenarioPlanPretty(plan) {
    const lines = [];
    ...
    if (plan.pendingPromptCount > 0) {
        lines.push("");
        lines.push(chalk.yellow(`${plan.pendingPromptCount} prompt${plan.pendingPromptCount === 1 ? "" : "s"} require interactive input.`));
    }
...
export function formatScenarioPlanNdjson(plan) {
...
export function formatScenarioPlanJson(plan) {
    return `${JSON.stringify(plan, null, 2)}\n`;
}
function formatStep(lines, step, options) {
    switch (step.kind) {
        case "command":
...
function formatCommandStep(lines, step, options) {
    ...
    for (const command of step.commands) {
        ...
        if (showTemplates && metadata.length > 0) {
            ...
            lines.push(indent("details: (hidden — use --plan-expand templates)", baseIndent + 4));
        }
        if (command.envDiff && command.envDiff.length > 0) {
            if (showEnvDiffs) {
                ...
                for (const diff of command.envDiff) {
                    ...
                    lines.push(indent(`${diff.key}=${diff.value} [${diff.source}]${previous}`, baseIndent + 6));
                }
            }
            else {
                lines.push(indent("env diffs: (hidden — use --plan-expand env)", baseIndent + 4));
            }
        }
    }
}
function formatPromptStep(lines, step, options) {
    const header = step.label ? `${step.label} (${step.id})` : step.id;
    const baseIndent = options.indent ?? 0;
...
function formatBranchStep(lines, step, options) {
    const header = step.label ? `${step.label} (${step.id})` : step.id;
    const baseIndent = options.indent ?? 0;
    lines.push(indent(`- [branch] ${header}`, baseIndent));
    if (options.preferences.expandBranches) {
        for (const branch of step.branches) {
            lines.push(indent(`• ${branch.expression} → ${branch.target} [${branch.result ? "true" : "false"}]`, baseIndent + 2));
        }
    }
    else if (step.branches?.length) {
        lines.push(indent("branch rationales: (hidden — use --plan-expand branches)", baseIndent + 2));
    }
...
function formatMessageStep(lines, step, options) {
    const header = step.label ? `${step.label} (${step.id})` : step.id;
    const baseIndent = options.indent ?? 0;
...
function formatComputeStep(lines, step, options) {
    const header = step.label ? `${step.label} (${step.id})` : step.id;
    const baseIndent = options.indent ?? 0;
...
function formatGroupStep(lines, step, options) {
    const header = step.label ? `${step.label} (${step.id})` : step.id;
    const baseIndent = options.indent ?? 0;
...
function formatIterateStep(lines, step, options) {
    const header = step.label ? `${step.label} (${step.id})` : step.id;
    const baseIndent = options.indent ?? 0;
...
function formatGitWorktreeGuardStep(lines, step, options) {
    const header = step.label ? `${step.label} (${step.id})` : step.id;
    const baseIndent = options.indent ?? 0;
    lines.push(indent(`- [git-worktree-guard] ${header}`, baseIndent));
    lines.push(indent(`status: ${step.status}${step.strategy ? ` (strategy: ${step.strategy})` : ""}`, baseIndent + 2));
    lines.push(indent(step.message, baseIndent + 2));
}
function formatPluginStep(lines, step, options) {
    const header = step.label ? `${step.label} (${step.id})` : step.id;
    const baseIndent = options.indent ?? 0;
    lines.push(indent(`- [plugin:${step.pluginType}] ${header} (plugin: ${step.pluginName})`, baseIndent));
    if (step.summary) {
